...
function export_array_to_excel(data, filename,  ints = false, merges = [], autoWidth = true) {
	function build_sheet(sheet_name, data) {
		...
		//invalid chars for sheet name \/*[]:? --max 31 chars
		sheet_name = replaceAll(sheet_name, '\\', '--');
		sheet_name = replaceAll(sheet_name, '/', '--');
		sheet_name = replaceAll(sheet_name, '*', '--');
		sheet_name = replaceAll(sheet_name, '[', '--');
		sheet_name = replaceAll(sheet_name, ']', '--');
		...
		if (autoWidth) {
			...
			const colWidth = data.map(row => row.map(val => {
				if (val == null || val == undefined) { /*First determine if it is null/undefined*/
					return {'wch': 10};
				} else {
			...
			/*Initial value of the first behavior*/
			let result = colWidth[0];
			for (let i = 1; i < colWidth.length; i++) {
				for (let j = 0; j < colWidth[i].length; j++) {
					if (!result[j]) result[j] = {'wch': 10};
					if (result[j]['wch'] < colWidth[i][j]['wch']) {
						//console.log(i, j, result[j]['wch'] +'<'+ colWidth[i][j]['wch']);
						result[j]['wch'] = colWidth[i][j]['wch'];
					}
				}
			}
