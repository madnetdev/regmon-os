...
function init_Forms_Selection_Menu(elem, type) {
	...
	var save1 = '<button type="button" class="save" style="margin:5px;">'+LANG.BUTTON_SAVE+' &nbsp; </button>';
	var save2 = '<button type="button" class="save" style="margin:5px;">'+LANG.BUTTON_SAVE+' &nbsp; </button>';
	...
	//Athlete_Forms_Select_Menu
		...
		$('.form_options').off('click').popover({
			...
			html: true,
			content: function () {
				var tmp = $(this).attr('data-ath_group_cat_form').split('|');
				var ath = tmp[0];
				var group = tmp[1];
				var cat = tmp[2];
				var form = tmp[3];
...
function reload_Opened_Forms_Selection_Menus(elem) {
	...
	if (current_Menu!='C_Athlete_Give_Forms_Access_To_Trainers' && $('#C_Athlete_Give_Forms_Access_To_Trainers').length && $('#C_Athlete_Give_Forms_Access_To_Trainers').text().trim()!='') {
...
function init_Athlete__Trainers_Select() {
	...
	$("#TRN_select").chosen({placeholder_text_single: LANG.SELECT_TRAINER, no_results_text: LANG.NO_RESULTS, search_contains: true, disable_search_threshold: 10});
...
function init_Trainer__Athletes_Select() {
	...
	$("#TRN_ATH_select").chosen({placeholder_text_single: LANG.SELECT_ATHLETE, no_results_text: LANG.NO_RESULTS, search_contains: true, disable_search_threshold: 10});
	//Trainer Athletes Select Change - load active Trainer FormsSelect
	$("#TRN_ATH_select").on('change', function () {
