...
var change_Current_User_Color;

jQuery(function() 
{

var st_all = '<span class="req">.</span>'; //all
var st_null = '<span class="req" title="'+LANG.R_STATUS.NO_REQUEST+'">.</span>'; //null
...
var st_leaveR = '<span class="req G_leaveR" title="'+LANG.R_STATUS.CANCELED_ACCESS_TRAINER+'">5</span>'; //5
var st_leaveA = '<span class="req G_leaveA" title="'+LANG.R_STATUS.CANCELED_ACCESS_ATHLETE+'">15</span>'; //15
var st_waitLR = '<span class="req G_waitLR" title="'+LANG.R_STATUS.REQ_WAIT_CANCELED_TRAINER+'">7</span>'; //7
...
$trainer_users.jqGrid({  
	...
	loadComplete: function(data) {
		...
		$('.checklist').off('click').on('click',function() {
			var athlete_id = $(this).attr('data-id');
...
function get_Athletes_IDS_Names(type) {
	athletes_ids = [];
	var ids = $trainer_users.jqGrid('getGridParam','selarrrow'); //selected rows
	var ids_uncheck = [];
	var header = LANG.REQUEST.NOBODY_SELECTED;
	var add_ID_name = function(id, status) {
		athletes_ids.push(id+'_'+status);
		names.push('<li>'+$trainer_users.jqGrid('getCell', id, 'u.firstname') +' '+ $trainer_users.jqGrid('getCell', id, 'u.lastname'));
	};
	if (ids.length>0) {
		var names = [];
		for (var i=0, i1=ids.length; i < i1; i++) {
			var status = $('#'+ids[i]+' td:last').text();
			if (type == 'request_access_athlete' && (status == '0' || status == '5' || status == '15' || status == '.')) {
				add_ID_name(ids[i], status);
				header = LANG.REQUEST.REQUEST_ACCESS_FROM;
			}
			else if (type == 'cancel_request_athlete' && (status == '7' || status == '17' || status == '8' || status == '9')) {
				add_ID_name(ids[i], status);
...
function init_request_athlete_buttons(el) {
	var el1, el2, bt, bt1, bt2;
		if (el == 'request_access_athlete') { el1 = 'cancel_request_athlete'; el2 = 'cancel_access_athlete'; }
	else if (el == 'cancel_request_athlete') { el1 = 'request_access_athlete'; el2 = 'cancel_access_athlete'; }
	else if (el == 'cancel_access_athlete') { el1 = 'request_access_athlete'; el2 = 'cancel_request_athlete'; }
	...
	$("#"+el).off('click').confirmation({
		...
		title: function() { return get_Athletes_IDS_Names(el); },
		...
		onConfirm: function(e, button) {
			if (athletes_ids.length != 0) {
				var data = {request: 'user2trainer', action: el, group_id: V_GROUP, athletes_ids: athletes_ids};
...
function Responsive_Trainer_Users() { 
    var p_width = $('#C_Request_Access_From_Athletes_link').prop('clientWidth');// Get width of parent container
    if (p_width == null || p_width < 1){
        p_width = $('#C_Request_Access_From_Athletes_link').prop('offsetWidth'); // For IE, revert to offsetWidth if necessary
    }
