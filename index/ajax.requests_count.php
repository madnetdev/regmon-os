...
if ($ADMIN OR $LOCATION_ADMIN OR $GROUP_ADMIN OR $GROUP_ADMIN_2) {
	//group requests
	$rows = $db->fetch("SELECT u2g.status AS req_status, u2g.modified, u2g.created, u.id, u.uname, u.lastname, u.firstname, u.sport, u.body_height, u.sex, u.birth_date, u.email, u.level 
...
elseif ($ATHLETE) {
	...
	WHERE u2g.group_id = ? AND u2g.status = 1 AND u2t.status > 5 AND u2t.status != 15 ORDER BY u2t.id", array($UID, $group_id)); 
}
